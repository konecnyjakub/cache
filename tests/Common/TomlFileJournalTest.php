...
#[TestSuite("TomlFileJournal")]
#[Group("journals")]
final class TomlFileJournalTest extends TestCase
{
    #[BeforeTestSuite]
    ...
    {
        (new TomlFileJournal(__DIR__ . DIRECTORY_SEPARATOR . "fileCache"))->clear();
        rmdir(__DIR__ . DIRECTORY_SEPARATOR . "fileCache");
    }

    public function testProcess(): void
    {
        $journal = new TomlFileJournal(__DIR__ . DIRECTORY_SEPARATOR . "fileCache");
        $key1 = "abc";
        $key2 = "def";
        ...
        $this->assertTrue(file_exists($journal->getFilename()));
        $this->assertSame("[abc]\nexpiresAt = 40\n\n[def]\nexpiresAt = 50", file_get_contents($journal->getFilename()));
        $this->assertSame([], iterator_to_array($journal->getKeysByTags(["tag1", ])));
        $metadata = $journal->get($key1);
