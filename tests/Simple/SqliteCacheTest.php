...
final class SqliteCacheTest extends TestCase
{
    public function testSingleKeyProcess(): void
    {
        ...
        $value = "def";
        $default = "default";
        $cache = new SqliteCache(new Sqlite("sqlite::memory:"));
    ...
    public function testDefaultTtl(): void
    {
        $key = "ttl";
        $value = "abc";
        $cache = new SqliteCache(new Sqlite("sqlite::memory:"), defaultTtl: -1);
    ...
    public function testNamespace(): void
    {
        ...
        $key1 = "one";
        $cache1 = new SqliteCache($pdo);
        $this->assertSame($key1, $cache1->getKey($key1));

        $key2 = "two";
        $namespace = "test";
        $cache2 = new SqliteCache($pdo, namespace: $namespace);
    ...
    public function testEvents(): void
    {
        ...
        $cache = new SqliteCache(new Sqlite("sqlite::memory:"), eventDispatcher: $eventDispatcher);
        $key = "one";
        $value = "abc";
        $cache->get($key);
        $cache->set($key, $value);
